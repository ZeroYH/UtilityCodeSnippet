...
-(id)initAudioPlayerWithFilePath:(NSString *)path sampleRate:(long)sample
{
    ...
    if (self) {
        NSData *audioData = [NSData dataWithContentsOfFile:path];
        [self audioPlayerWithData:audioData];
        NSLog(@"nihao");
    }
    return self;
}
...
- (void)audioPlayerWithData:(NSData *)data {
    self.pcmData = [NSMutableData dataWithData:data];
    NSError *err = nil;
    self.player = [[AVAudioPlayer alloc]initWithData:self.pcmData error:&err];
    if (err)
    {
        NSLog(@"%@",err.localizedDescription);
        if (_delegate != nil && [_delegate respondsToSelector:@selector(pcmPlayerError:)]) {
            [_delegate pcmPlayerError:err.localizedDescription];
        }
    }
    self.player.delegate = self;
    [self.player prepareToPlay];
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag
{
    ...
    self.isPlaying=NO;
    if (_delegate != nil && [_delegate respondsToSelector:@selector(pcmPlayerDidFinishPlaying:successfully:)]) {
        [_delegate pcmPlayerDidFinishPlaying:player successfully:flag];
    }
}
...
- (NSTimer *)timer {
    if (!_timer) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(listeningVolume) userInfo:nil repeats:YES];
    }
    return _timer;
}
